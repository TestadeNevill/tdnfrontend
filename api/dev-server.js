...
import { handleMapsApi, LAYER_HANDLERS } from "./_lib/mapsRouter.js";
...
export async function handleLabsApi(req, res, url) {
  ...
  const mapsMatch = url.pathname.match(/^\/api\/labs\/maps\/([a-z-]+)$/);
  if (mapsMatch && (req.method === "POST" || req.method === "GET")) {
    return handleMapsApi(req, res, mapsMatch[1]);
  }
